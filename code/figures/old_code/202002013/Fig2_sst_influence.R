...
# Read data
fe <- read.csv(file.path(datadir, "pella_best_fixed_sst_results.csv"))
re <- read.csv(file.path(datadir, "pella_best_random_sst_results.csv"))
...
# Setup figure
figname <- "Fig2_sst_influence.png"
...
xlabel <- "Influence of sea surface temperature"
...
# Example stocks
ex_stocks <- c("COD4TVn", "GUACORPERU", "NFURSEALSMI")
ex_text <- c("Atlantic cod, NAFO 4TVn", "Guanay cormorant, Peru", "Fur seal, San Miguel Island")
...
xmax <- c(1, 1, 1)
a <- data %>% 
  filter(stockid%in%ex_stocks) %>% 
  group_by(stockid) %>% 
  summarize(ymin=min(sp_sd),
            ymax=max(sp_sd))
ymin <- c(-0.1, -0.8, -0.8)
ymax <- c(0.4, 0.4, 0.2)
ybin <- c(0.1, 0.2, 0.2)
