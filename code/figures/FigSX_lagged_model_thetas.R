...
stockid_order <- data_orig %>% 
  # Composite dataset
  filter(dataset=="composite lag-2" & framework=="fixed" & covariate=="cprey") %>% 
...
data <- data_orig %>% 
  # Composite dataset
  filter(dataset=="composite lag-2" & covariate!="sst") %>% 
  # Format framework
  ...
  mutate(covariate=recode_factor(covariate, 
                                 # Primary prey
                                 "prey1"="Primary prey\n(no lag)",
                                 "prey1 lag-1"="Primary prey\n(1-yr lag)",
                                 "prey1 lag-2"="Primary prey\n(2-yr lag)",
                                 # Composite prey
                                 "cprey"="Composite prey\n(no lag)",
                                 "cprey lag-1"="Composite prey\n(1-yr lag)",
                                 "cprey lag-2"="Composite prey\n(2-yr lag)")) %>% 
  # Set stock order
...
perc_df <- data %>% 
  ...
  mutate(yval=recode(type, "birds"=36.5, "mammals"=43.5, "fish"=27.5) %>% as.numeric())
...
g <- ggplot(data, aes(x=betaT, y=stockid, col=betaT_inf)) +
  ...
  # Add lines
  geom_line(data_ci, mapping=aes(x=value_cap, y=stockid, col=betaT_inf), alpha=0.5, lwd=0.5) +
  # Add points
  geom_point(size=0.5) +
  # Add vertical line
  geom_vline(xintercept=0, color="grey20", lwd=0.2) +
  # Add horizontal lines seperating groups
  geom_hline(yintercept=c(28.5,37.5), linetype="dotted", color="grey30") +
  # Add percent labels
  geom_text(data=perc_df, mapping=aes(x=-10, y=yval, label=p_neg_sig), color="red", size=1.5, hjust=0, alpha=0.65) +
  geom_text(data=perc_df, mapping=aes(x=-10, y=yval-1, label=p_neg), color="grey40", size=1.5, hjust=0, alpha=0.65) +
  geom_text(data=perc_df, mapping=aes(x=10, y=yval, label=p_pos_sig), color="blue", size=1.5, hjust=1, alpha=0.65) +
  geom_text(data=perc_df, mapping=aes(x=10, y=yval-1, label=p_pos), color="grey40", size=1.5, hjust=1, alpha=0.65) +
  # Constrain xlim
  # xlim(-4,4) +
  # Legend and labels
...
# Export figure
ggsave(g, filename=file.path(plotdir, "FigSX_lagged_model_thetas.png"), 
