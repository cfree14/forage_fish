...
sp.perfect <- c("Bigeye tuna Atlantic Ocean", "Swordfish Northern Atlantic",
                "Yellowfin tuna Atlantic Ocean")
sp.nls.fail <- nlsfits$stocklong[is.na(nlsfits$k)]
problem.stocks <- c(sp.perfect, sp.nls.fail)
...
if(FALSE){
  dyn.unload(paste(tmbdir, dynlib("spmodel_no_prey_link"), sep="/"))
  file.remove(paste(tmbdir, c("spmodel_no_prey_link.o", "spmodel_no_prey_link.dll"), sep="/"))
  compile("spmodel_no_prey_link.cpp")
}

# Load TMB code
dyn.load(dynlib("spmodel_no_prey_link"))

# Input data and parameter starting values
b0_starts <- log(sapply(stocks, function(x) max(data$n_sd[data$stocklong==x])) * 1.5)
...
# Initialization
model <- MakeADFun(data=input.data, parameters=params, DLL="spmodel_no_prey_link")
...
save(data, stocks, problem.stocks, nstocks, 
     ...
     file=paste(outputdir, "hilborn_etal_2017_spmodel_no_prey_link.Rdata", sep="/"))
