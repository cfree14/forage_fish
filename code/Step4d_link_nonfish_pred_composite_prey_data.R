...
data <- bm_pred_ts %>%
  ...
  rename(prey_stocks=prey_stocks_all, 
         prey_b=biomass, prey_lag1_b=biomass_lag1, prey_lag2_b=biomass_lag2, 
  ...
  mutate(n_sd=n/max(n),
         ...
         prey_lag2_b_sd=scale(prey_lag2_b)) %>% # scale using z-score
  # Reduce columns
  select(stockid, stocklong, year, catch, n, n_units, sp, sp_units, 
         prey1, prey_impt, prey_stocks, prey_b, prey_lag1_b, prey_lag2_b,
         prey_btype, prey_bunits,
         n_sd, sp_sd, prey_b_sd, prey_lag1_b_sd, prey_lag2_b_sd)
