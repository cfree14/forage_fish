...
props1 <- dcast(props, diet_id + pred_comm + region + prey_comm + nrefs + references ~ diet_prop_units, value.var="prop")

# Rearrange and add prop to use columns
# weight > number > occur > model > unknown
props2 <- props1 %>% 
  select(diet_id, pred_comm, region, prey_comm, nrefs, references, 
